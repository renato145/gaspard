...
# %% auto 0
__all__ = ['UsageMetadata', 'empty', 'models', 'find_block', 'contents', 'usage', 'mk_msgs', 'Client', 'get_stream',
...
def json2proto(schema_dict):
    def convert_type(t):
        ...
        }.get(t, protos.Type.TYPE_UNSPECIFIED)
    
    def convert_property(prop, depth=0):
        schema = protos.Schema(type=convert_type(prop.get('type')))
        if 'items' in prop:
            if '$ref' in prop['items']:
                ref_name = prop['items']['$ref'].split('/')[-1]
                schema.items = convert_property(schema_dict['input_schema']['$defs'][ref_name], depth+1)
            else:
                schema.items = convert_property(prop['items'], depth+1)
        if 'properties' in prop:
            for k, v in prop['properties'].items(): schema.properties[k] = convert_property(v, depth+1)
        if 'required' in prop: schema.required.extend(prop['required'])
        return schema
    
    return convert_property(schema_dict['input_schema'])

# %% ../00_core.ipynb
def cls2tool(c) -> genai.protos.FunctionDeclaration:
    json_schema = get_schema(c)
    schema = create_proto_schema(json_schema)
...
# %% ../00_core.ipynb
def _convert_proto(obj):
    if isinstance(obj, dict):
        return {k: _convert_proto(v) for k, v in obj.items()}
    elif isinstance(obj, list) or isinstance(obj, RepeatedComposite):
        return [_convert_proto(v) for v in obj]
    elif isinstance(obj, MapComposite):
        return {k: _convert_proto(v) for k, v in obj.items()}
    elif hasattr(obj, 'DESCRIPTOR'):  # Proto Message
        return {k.name: _convert_proto(getattr(obj, k.name)) for k in obj.DESCRIPTOR.fields}
    else:
        return obj
